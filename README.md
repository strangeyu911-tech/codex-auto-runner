...
你只需要在 Codex 里正常干活。Codex Auto Runner 会识别当前会话、记录任务状态、等待额度恢复、二次确认额度确实可用，然后优先恢复原 Codex 线程，让 Codex 继续沿着原上下文往前推进。

如果原线程的写权限正被另一个 Codex 进程占用（最常见的是常驻在托盘里的 Windows 桌面版），它不会傻等对方放锁，而是从原线程 fork 出一条保持上下文与任务血缘的续跑分支继续推进。
...
- 你希望恢复窗口到来时自动续跑原线程（写权限被其他 Codex 进程占用时，自动 fork 出等价的续跑分支），无论这个会话有没有开启目标模式。
- 你希望撞上 5h 限额后不必守着屏幕挑线程：被额度打断的会话会被自动接管。
...
- 你使用 macOS 或 Linux。当前实现只面向 Windows。
...
7. 向 Codex 发送最小必要的继续指令 —— 首轮带上原目标、补充说明与验收标准：

```text
继续任务。

这是之前会话的延续，你之前的历史上下文都还在；
请从中断的地方接着做，不要重头开始、不要换话题、不要重复询问已经得到过的信息。
```

第 2 轮起只发一句 `继续任务。`：完整指令上一轮已经写进 thread 历史了，
每轮整份重发只会把对话刷成一堵墙。

8. Codex 读取原上下文继续推进。**它正常说话就行** —— CAR 不需要模型输出 JSON：

- 结构化结论由模型写进项目根目录的 `.car/status.json`（已写进 `.git/info/exclude`，
  回合开始前会被清空，避免把上一轮的陈旧结论当成本轮结果）；
- 拿不到文件时，回退到读取回复正文里的 JSON（兼容旧线程）；
- 再拿不到，就用 app-server 的原生信号兜底：`thread/goal/get` 的
  `blocked` / `complete`，以及 `thread/status/changed` 的
  `activeFlags: [waitingOnUserInput | waitingOnApproval]`。

   为什么不用 `turn/start` 的 `outputSchema`：它的定义是「约束**最终那条 assistant 消息**」，
   而那条消息就是用户在桌面版里看到的那条 —— 要求 JSON 等于把一坨结构化数据糊到用户脸上。
...
- **会话自动接管**：周期性扫描 Codex 会话，发现「最近一个 turn 确实被额度打断、且尚未被接管」的线程时自动建任务续跑 —— 撞限额后不必醒来挑线程。只捡真正因额度失败的会话；正常结束、被主动中断、已取消过的一律不动。默认开启，`CAR_DISCOVERY=0` 可关闭。
- **优先续跑最新**：同时存在多条被限额中断的线程时，优先续跑**最新被打断的那条**。显式设置的优先级仍然优先。
- **重启后自动接上**：守护进程被重启/被杀时挂在运行中的任务，不再一律停成「待你确认」。会读线程的 turn 历史区分两种命运 —— 最后一个 turn 被掐断（工作没做完）就自动放回队列续跑；只有「turn 其实已经跑完」的才交回给人。读不到线程历史时保守不动。
- **自动恢复 active 目标**：目标因暂停、受限或额度耗尽停住时，可在恢复运行前重新设为 active。
- **原线程续跑 + 写锁冲突降级**：优先恢复原 Codex thread，让模型读取原上下文继续，而不是新开一个空会话；若原线程的写权限被其他 Codex 进程占用，则自动 fork 出一条上下文与任务血缘连续的续跑分支。
...
- **会话自动接管**：撞限额的会话被自动识别并建任务续跑，无需手动挑线程。
- **限额打断识别与优先续跑**：记录被打断的线程与时间，多条并存时优先续跑最新的一条。
- 原线程续跑（写锁冲突时自动 fork 续跑分支）与目标 active 恢复。
- 自动版任务创建与专业版任务配置。
- 本地额度仪表盘与中英文界面。
- 周额度耗尽后的重置次数续跑选项。
