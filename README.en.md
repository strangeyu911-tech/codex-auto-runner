...
- **Automatic catch-up after a restart**: tasks that were mid-run when the daemon was killed are no longer parked as "needs your confirmation". The runner reads the thread's turn history to tell the two cases apart — if the last turn was cut off (work unfinished) it goes straight back into the queue; only a last turn that actually completed is handed back to a human. When the thread history cannot be read, it stays conservative and does nothing.
